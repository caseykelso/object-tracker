        ...
        while (running) {
            ...
            // Draw objects
            {
                ...
                for (const auto& obj : objects) {
                    ...
                    infoSS << std::fixed << std::setprecision(1) 
                           << "(" << obj.x << "," << obj.y << ") " 
...
public:
    Visualizer3D(int width = 80, int height = 40,
    ...
    void addObject(const Object3D& obj) {
        ...
        auto it = std::find_if(objects.begin(), objects.end(),
                               [&obj](const Object3D& o) { return o.id == obj.id; });
...
int main() {
    try {
        ...
        while (viz.isRunning() && frame < 1000) {
            ...
            viz.updateObjects({
                Object3D(-2.0 + sin(frame * 0.02), 1.0 + cos(frame * 0.01), 0.0, 1.0, 0.5, 1),
            ...
            if (frame % 100 == 0) {
                viz.addObject(Object3D(
                    -4.0 + static_cast<double>(rand()) / RAND_MAX * 8.0,
                    -2.0 + static_cast<double>(rand()) / RAND_MAX * 4.0,
                    0.0,
...
Visualizer3D* createVisualization(double minX = -5.0, double maxX = 5.0,
                                double minY = -5.0, double maxY = 5.0) {
